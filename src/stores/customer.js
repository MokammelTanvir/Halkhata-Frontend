...
export const useCustomerStore = defineStore("customer", {
  ...
  actions: {
    ...
    async getCustomer(customer_id) {
      this.is_loading = true;
      try {
        const { data } = await inventoryAxiosClient.get(
    ...
    async storeCustomer(formData) {
      this.is_loading = false;
      try {
        const { data } = await inventoryAxiosClient.post(
          "/customers",
          formData
        );
        console.log(data);
    ...
    async updateCustomer(formData, customer_id) {
      this.is_loading = false;
      try {
        const { data } = await inventoryAxiosClient.put(
          `/customers/${customer_id}`,
          formData
        );
        console.log(data);
        ...
        this.getCustomers(this.pagination.current_page, this.dataLimit);
        this.is_loading = false;
        this.router.push({ name: "customer-index" });
      } catch (error) {
        this.is_loading = false;
    ...
    async deleteCustomer(customer_id, callback) {
      this.is_loading = false;
      try {
        const { data } = await inventoryAxiosClient.delete(
          `/customers/${customer_id}`
        );
        callback("success");
        this.swal({
